...
(function(window,document){
	...
	CHEER.namespace("CHEER.Cookie");
	CHEER.Cookie = (function(){
		return {
			set: function(label, value, expireTime, subdomain){
				if(CHEER.isNull(subdomain) || CHEER.isUndef(subdomain)){
					subdomain = '';
				}
				var cookie = label + "=" + escape(value) + "; domain=" + subdomain + "xunlei.com; path=/;";
				if (CHEER.isUndef(expireTime)){
					document.cookie = cookie;
				}
				else{
					var expires = new Date();
					expires.setTime(expires.getTime() + expireTime*1000);
					document.cookie = label + "=" + escape(value) +"; domain="+subdomain+".xunlei.com; path=/; expires=" + expires.toGMTString() + ";";
				}
			},

			get: function(label){
				return CHEER.isNull(document.cookie.match(new RegExp("(^"+label+"| "+label+")=([^;]*)"))) ? "" : unescape(RegExp.$2);
			},
	...
	CHEER.namespace("CHEER.URL");
	CHEER.URL = (function(){
		return {
			getParameter: function(parameter, url){
				url = url || win.location.href;
				var _param = CHEER.isNull(url.match(new RegExp("[?#&]"+parameter+"=(.*?)(?:[#&]|$)", "i"))) ? "" : RegExp.$1;
				return _param;
			},
			...
			getProtocol: function(url){
				return CHEER.isNull(url.match(/^(\w+):\/\//)) ? "" : RegExp.$1;
			},

			getHost: function(url){
				return CHEER.isNull(url.match(/:\/\/([a-zA-Z0-9.]+)/)) ? "" : RegExp.$1;
			},

			isLocal: function(url){
				return !CHEER.isNull(url.match(/(^file|^[A-Za-z]):/i));
			}
		};
	})();
